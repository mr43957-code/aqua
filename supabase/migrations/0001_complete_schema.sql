...
-- ============================================================
-- إضافة سلايدر سفلي وجدول الشركاء/الموردين
-- ============================================================

-- سلايدر سفلي (ترويجي)
insert into public.sliders (name, position, is_active, autoplay, autoplay_speed, show_arrows, show_dots)
values ('السلايدر الترويجي السفلي', 'home_bottom', true, true, 6000, false, true)
on conflict do nothing;
...
create policy "public_read_partners" on public.partners for select using (is_active = true or public.is_admin());
create policy "admin_write_partners" on public.partners for all using (public.is_admin()) with check (public.is_admin());
