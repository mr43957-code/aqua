...
const navGroups: { title: string; items: NavItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/admin/partners', label: 'الشركاء والموردون', icon: Users },
    ],
  },
  {
    title: 'العمليات',
    ...
    <div className={clsx('flex flex-col h-full', collapsed ? 'w-16' : 'w-64')}>
      ...
      <div className="flex items-center gap-3 px-4 py-5 border-b border-white/10">
        ...
        <button
          ...
          className="mr-auto text-primary-200 hover:text-white hidden lg:block transition"
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-4">
        {navGroups.map((group) => (
          <div key={group.title}>
            {!collapsed && (
              <p className="text-primary-300 text-[10px] font-semibold uppercase tracking-wider px-3 mb-1">
                {group.title}
              </p>
            )}
              ...
              {group.items.map((item) => (
                <Link
                  ...
                  className={clsx(
                    ...
                    isActive(item.href)
                      ? 'bg-white text-primary-800 font-semibold shadow-md'
  ...
  return (
    <>
      ...
      <aside className={clsx(
        'hidden lg:flex flex-col bg-gradient-to-b from-primary-950 via-primary-900 to-primary-950 transition-all duration-300 flex-shrink-0',
        collapsed ? 'w-16' : 'w-64'
      )}>
        <SidebarContent />
      </aside>
      ...
      {/* Mobile toggle */}
      <button
        className="lg:hidden fixed bottom-6 right-6 z-50 bg-gradient-to-br from-primary-600 to-primary-800 text-white p-3 rounded-full shadow-lg hover:scale-110 transition-all duration-200"
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative bg-gradient-to-b from-primary-950 to-primary-900 w-64 flex flex-col h-full animate-slide-in">
            <button
              onClick={() => setMobileOpen(false)}
              className="absolute top-4 left-4 text-primary-200 hover:text-white transition"
              aria-label="إغلاق"
            >
              <X className="w-5 h-5" />
            </button>
